...
async def _autonomous_tick():
    ...
    # keep the crew MOVING into the dark: any living crewmate not actively working or walking to a task is
    # sent to relight the nearest dark room (overriding a passive follow/whisper/idle). This stops them
    # huddling on one side — they constantly sweep the dark rooms, so bodies get found fast and the station
    # stays lit. Crew already en route to / working a console are left alone.
    if g.open_tasks:
        for a in g.living(impostor=False):
            if a.work == 0 and a.dest is None:
